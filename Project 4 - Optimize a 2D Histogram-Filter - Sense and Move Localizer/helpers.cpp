...
/* Normalize involves a total probability that is calculated but that does not equal 1.0 .  By dividing into a normalized sum we will have a
Total probability that sums to 1.0 .
*/

vector< vector<float> > normalize(vector< vector <float> > grid) {

	// Declaring a new 2D Vector called newGrid for the normalized proabilities  
...
vector < vector <float> > blur(vector <vector <float> > grid, float blurring) {
	...
	float center_prob = 1.0f - blurring; // f tells compiler this is a floating point number
		...
		for (int j = 0; j < columns; j++)
		{
			...
			for (int dx = -1; dx < 2; dx++)
			{
				for (int dy = -1; dy < 2; dy++)
				{
					...
					int new_j = (j + dx + columns) % columns;
